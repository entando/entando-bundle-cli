...
export class CustomResourceService {
  ...
  public constructor(image:string, tags:string[], digests:Map<string,string>, yamlDescriptor:YamlBundleDescriptor) {
    this.image = image
    this.tags = tags
  ...
  public createCustomResource(): CustomResourceDescriptor {
    ...
    const crDescriptor:CustomResourceDescriptor = {
      ...
      kind: KIND,
      metadata : {
        name: this.yamlDescriptor.name + "-" + BundleService.generateBundleId(this.image),
        labels : {
          plugin: false,
          widget: false,
    ...
    if(this.yamlDescriptor.components.widgets.length > 0 || this.yamlDescriptor.components["app-builder"].length > 0) {
